...
class RawPCM(Modulation):
    def __init__(self, path: str, sampling_freq: float, mod_freq_div: int):
        super().__init__()
        ModulationAudioFile().dll.AUTDModulationRawPCM(
            byref(self.ptr), path.encode("utf-8"), sampling_freq, mod_freq_div
        )
...
class Wav(Modulation):
    def __init__(self, path: str, mod_freq_div: int):
        super().__init__()
        ModulationAudioFile().dll.AUTDModulationWav(
            byref(self.ptr), path.encode("utf-8"), mod_freq_div
        )
