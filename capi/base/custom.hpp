...
class CustomGain final : public autd3::Gain {
 public:
  ...
  std::vector<autd3::driver::Drive> calc(const autd3::core::Geometry& geometry) override {
    std::vector<autd3::driver::Drive> drives(geometry.num_transducers(), autd3::driver::Drive{autd3::driver::Phase(0), autd3::driver::Amp(0)});
    std::transform(_phase.begin(), _phase.end(), _amp.begin(), drives.begin(), [](const auto phase, const auto amp) {
      return autd3::driver::Drive{autd3::driver::Phase(phase), autd3::driver::Amp(amp)};
    });
    return drives;
  }
...
class CustomModulation final : public autd3::Modulation {
 public:
  ...
  explicit CustomModulation(const autd3_float_t* buffer, const size_t size, const uint32_t freq_div = 40960) : Modulation() {
    _freq_div = freq_div;
    _data.resize(size);
    std::copy_n(reinterpret_cast<const autd3::Amp*>(buffer), size, _data.begin());
  }

  std::vector<autd3::Amp> calc() override { return _data; }
