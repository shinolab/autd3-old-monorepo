...
void AUTDLinkEmulator(void** out, const uint16_t port, const void* cnt) {
  ...
  auto* link = link_create(autd3::link::Emulator<autd3::core::NormalTransducer>(p_cnt->geometry()).port(port).build());
  *out = link;
}
...
void AUTDLinkEmulatorLegacy(void** out, const uint16_t port, const void* cnt) {
  ...
  auto* link = link_create(autd3::link::Emulator<autd3::core::LegacyTransducer>(p_cnt->geometry()).port(port).build());
  *out = link;
}
