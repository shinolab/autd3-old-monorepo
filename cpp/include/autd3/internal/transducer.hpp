...
class Transducer {
 public:
  ...
  void set_frequency(const double freq) const {
    if (char err[256]{}; !native_methods::AUTDSetTransFrequency(_ptr, _idx, freq, err)) throw AUTDException(err);
  }
  ...
  void set_mod_delay(const uint16_t delay) const { native_methods::AUTDSetTransModDelay(_ptr, _idx, delay); }
  ...
  void set_cycle(const uint16_t cycle) const {
    if (char err[256]; !native_methods::AUTDSetTransCycle(_ptr, _idx, cycle, err)) throw AUTDException(err);
  }
