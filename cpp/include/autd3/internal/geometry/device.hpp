...
class Device {
  class DeviceView : public std::ranges::view_interface<DeviceView> {
   public:
    DeviceView() = default;
    explicit DeviceView(const std::vector<Transducer>& vec)
        : _begin(vec.cbegin()), _end(vec.cend()) {}
 ...
 public:
  explicit Device(const size_t idx, const native_methods::DevicePtr ptr)
      : _idx(idx), _ptr(ptr) {
    const auto size = static_cast<size_t>(AUTDDeviceNumTransducers(_ptr));
    _transducers.clear();
    _transducers.reserve(size);
    for (uint32_t i = 0; i < size; i++) _transducers.emplace_back(idx, i, _ptr);
  }
  ...
  [[nodiscard]] double sound_speed() const {
    return AUTDDeviceGetSoundSpeed(_ptr);
  }
  ...
  void set_sound_speed(const double value) const {
    AUTDDeviceSetSoundSpeed(_ptr, value);
  }
   ...
   * @param m Molar mass
   */
  void set_sound_speed_from_temp(const double temp, const double k = 1.4,
   ...
   */
  [[nodiscard]] double attenuation() const {
    return AUTDDeviceGetAttenuation(_ptr);
  }

  /**
   * @brief Set attenuation coefficient
   */
  void set_attenuation(const double value) const {
    AUTDDeviceSetAttenuation(_ptr, value);
  }
   ...
   * @param value
   */
  void reads_fpga_info(const bool value) const {
    AUTDDeviceSetReadsFPGAInfo(_ptr, value);
  }
  ...
  [[nodiscard]] DeviceView transducers() const noexcept {
    return DeviceView(_transducers);
  }

  [[nodiscard]] std::vector<Transducer>::const_iterator cbegin()
      const noexcept {
    return _transducers.cbegin();
  }
  [[nodiscard]] std::vector<Transducer>::const_iterator cend() const noexcept {
    return _transducers.cend();
  }
  ...
  [[nodiscard]] const Transducer& operator[](const size_t i) const {
