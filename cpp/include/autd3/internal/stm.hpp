...
class STM {
 ...
 protected:
  [[nodiscard]] native_methods::STMPropsPtr props() const {
    ...
    if (_period.has_value()) ptr = native_methods::AUTDSTMPropsNewWithPeriod(static_cast<uint64_t>(_period.value().count()));
    if (_config.has_value()) ptr = AUTDSTMPropsNewWithSamplingConfig(static_cast<native_methods::SamplingConfiguration>(_config.value()));
...
class FocusSTM final : public STM {
 public:
  ...
  static FocusSTM new_with_sampling_configuration(const SamplingConfiguration config) { return FocusSTM(std::nullopt, std::nullopt, config); }

  template <typename Rep, typename Period>
  static FocusSTM new_with_period(const std::chrono::duration<Rep, Period> period) {
...
class GainSTM final : public STM {
 public:
   ...
   * @return GainSTM
   */
  static GainSTM new_with_sampling_config(const SamplingConfiguration config) { return GainSTM(std::nullopt, std::nullopt, config); }

  template <typename Rep, typename Period>
  static GainSTM new_with_period(const std::chrono::duration<Rep, Period> period) {
