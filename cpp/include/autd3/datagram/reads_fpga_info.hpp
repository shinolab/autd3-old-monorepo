...
template <class F>
concept configure_reads_fpga_info_f = requires(F f, const internal::geometry::Device& d) {
...
class ConfigureReadsFPGAInfo final {
  using native_f = bool (*)(const void*, internal::native_methods::GeometryPtr, uint32_t);

 public:
  explicit ConfigureReadsFPGAInfo(const F& f) : _f(f) {
    _f_native = +[](const void* context, const internal::native_methods::GeometryPtr geometry_ptr, const uint32_t dev_idx) -> bool {
      ...
      return static_cast<const ConfigureReadsFPGAInfo*>(context)->_f(dev);
    };
  }

  [[nodiscard]] internal::native_methods::DatagramPtr ptr(const internal::geometry::Geometry& geometry) const {
    return AUTDDatagramConfigureReadsFPGAInfo(const_cast<void*>(reinterpret_cast<const void*>(_f_native)),
