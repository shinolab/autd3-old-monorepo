...
TEST(Internal, EmitIntensityWithCorrection) {
  for (int i = 0; i <= 0xFF; i++) {
    const auto intensity = autd3::internal::EmitIntensity::new_with_correction(static_cast<uint8_t>(i));
    ASSERT_EQ(intensity.value(),
              static_cast<uint8_t>(
                  std::round(std::asin(std::powf((static_cast<double>(i) / 255.0), 1.0 / autd3::internal::native_methods::DEFAULT_CORRECTED_ALPHA)) /
