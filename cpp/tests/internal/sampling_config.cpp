...
TEST(Internal, SamplingConfigWithFreqDiv) {
  const auto config = autd3::internal::SamplingConfiguration::new_with_frequency_division(512);
  ASSERT_EQ(config.frequency_division(), 512);
  ASSERT_EQ(config.frequency(), 40e3);
  ASSERT_EQ(config.period(), std::chrono::microseconds(25));

  ASSERT_THROW((void)autd3::internal::SamplingConfiguration::new_with_frequency_division(SAMPLING_FREQ_DIV_MIN - 1), autd3::internal::AUTDException);
}

TEST(Internal, SamplingConfigWithFreq) {
  const auto config = autd3::internal::SamplingConfiguration::new_with_frequency(40e3);
  ASSERT_EQ(config.frequency_division(), 512);
  ASSERT_EQ(config.frequency(), 40e3);
  ASSERT_EQ(config.period(), std::chrono::microseconds(25));

  ASSERT_THROW((void)autd3::internal::SamplingConfiguration::new_with_frequency(FREQ_MIN - 0.1), autd3::internal::AUTDException);
  ASSERT_THROW((void)autd3::internal::SamplingConfiguration::new_with_frequency(FREQ_MAX + 0.1), autd3::internal::AUTDException);
}

TEST(Internal, SamplingConfigWithPeriod) {
  const auto config = autd3::internal::SamplingConfiguration::new_with_period(std::chrono::microseconds(25));
  ASSERT_EQ(config.frequency_division(), 512);
  ASSERT_EQ(config.frequency(), 40e3);
  ASSERT_EQ(config.period(), std::chrono::microseconds(25));

  ASSERT_THROW((void)autd3::internal::SamplingConfiguration::new_with_period(std::chrono::nanoseconds(PERIOD_MIN - 1)),
               autd3::internal::AUTDException);
  ASSERT_THROW((void)autd3::internal::SamplingConfiguration::new_with_period(std::chrono::nanoseconds(PERIOD_MAX + 1)),
               autd3::internal::AUTDException);
}
