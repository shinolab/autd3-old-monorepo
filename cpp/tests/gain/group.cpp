...
TEST(Gain, Group) {
  ...
  ASSERT_TRUE(
      autd.send_async(autd3::gain::Group([cx](const auto&, const auto& tr)
                                             -> std::optional<const char*> {
                        if (tr.position().x() < cx) return "uniform";
                        return "null";
                      })
                          .set("uniform", autd3::gain::Uniform(0x80).with_phase(
                                              autd3::internal::pi))
                          .set("null", autd3::gain::Null()))
          .get());

  for (auto& dev : autd.geometry()) {
    auto [intensities, phases] =
...
TEST(Gain, GroupUnkownKey) {
  ...
  try {
    autd.send_async(
            autd3::gain::Group(
                [](const auto&, const auto&) -> std::optional<const char*> {
                  return "null";
                })
                .set("uniform",
                     autd3::gain::Uniform(0x80).with_phase(autd3::internal::pi))
                .set("null", autd3::gain::Null()))
        .get();
...
TEST(Gain, GroupCheckOnlyForEnabled) {
  ...
  ASSERT_TRUE(
      autd.send_async(autd3::gain::Group(
                          [&check](const auto& dev,
                                   const auto& tr) -> std::optional<int> {
                            check[dev.idx()] = true;
                            return 0;
                          })
                          .set(0, autd3::gain::Uniform(0x80).with_phase(
                                      autd3::internal::pi)))
          .get());
