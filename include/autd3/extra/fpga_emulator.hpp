...
constexpr driver::float_t TRANS_SIZE_FIXED_POINT_UNIT = 40000 / driver::METER;
...
constexpr size_t ADDR_MOD_MEM_SEGMENT = 0x0020;
...
constexpr size_t ADDR_STM_MEM_SEGMENT = 0x0050;
...
class FPGA {
 public:
  ...
  [[nodiscard]] uint16_t read(const uint16_t addr) const {
    ...
    switch (select) {
      ...
      case fpga::BRAM_SELECT_MOD:
        return _modulator_bram[static_cast<size_t>(_controller_bram[fpga::ADDR_MOD_MEM_SEGMENT]) << 14 | addr_in_bram];
      case fpga::BRAM_SELECT_NORMAL:
        return _normal_op_bram[addr_in_bram];
      case fpga::BRAM_SELECT_STM:
        return _stm_op_bram[static_cast<size_t>(_controller_bram[fpga::ADDR_STM_MEM_SEGMENT]) << 14 | addr_in_bram];
      default:
        return 0;
    }
  }
  ...
  void write(const uint16_t addr, const uint16_t data) {
    ...
    switch (select) {
      case fpga::BRAM_SELECT_CONTROLLER:
        ...
        break;
      case fpga::BRAM_SELECT_MOD:
        _modulator_bram[static_cast<size_t>(_controller_bram[fpga::ADDR_MOD_MEM_SEGMENT]) << 14 | addr_in_bram] = data;
        break;
      case fpga::BRAM_SELECT_NORMAL:
        _normal_op_bram[addr_in_bram] = data;
        break;
      case fpga::BRAM_SELECT_STM:
        _stm_op_bram[static_cast<size_t>(_controller_bram[fpga::ADDR_STM_MEM_SEGMENT]) << 14 | addr_in_bram] = data;
        break;
      default:
        break;
    }
  ...
  [[nodiscard]] bool is_outputting() const {
    ...
    if (!is_stm_mode()) {
      ...
      return std::any_of(duties.begin(), duties.end(), [](const uint16_t x) { return x != 0; });
    }
    return true;
  }

  [[nodiscard]] std::pair<std::vector<uint16_t>, std::vector<uint16_t>> drives(const size_t idx) const {
    if (is_stm_mode()) {
      if (is_stm_gain_mode()) {
 ...
 private:
  ...
  [[nodiscard]] std::vector<uint16_t> advanced_duty() const {
  ...
  [[nodiscard]] std::vector<uint16_t> advanced_phase() const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
  ...
  [[nodiscard]] std::vector<uint16_t> legacy_duty() const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
  ...
  [[nodiscard]] std::vector<uint16_t> legacy_phase() const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
    for (size_t i = 0; i < _num_transducers; i++) {
  ...
  [[nodiscard]] std::vector<uint16_t> gain_stm_advanced_duty(const size_t idx) const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
  ...
  [[nodiscard]] std::vector<uint16_t> gain_stm_advanced_phase(const size_t idx) const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
  ...
  [[nodiscard]] std::vector<uint16_t> gain_stm_legacy_duty(const size_t idx) const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
  ...
  [[nodiscard]] std::vector<uint16_t> gain_stm_legacy_phase(const size_t idx) const {
    std::vector<uint16_t> d;
    d.reserve(_num_transducers);
    for (size_t j = 0; j < _num_transducers; j++) {
  ...
  [[nodiscard]] std::vector<uint16_t> focus_stm_duty(const size_t idx) const {
  ...
  [[nodiscard]] std::vector<uint16_t> focus_stm_phase(const size_t idx) const {
    const auto ultrasound_cycles = cycles();
