...
constexpr uint32_t TR_POS[NUM_TRANS_IN_UNIT] = {
...
class FPGA {
 public:
  ...
  void init() { _controller_bram[ADDR_VERSION_NUM] = static_cast<uint16_t>(ENABLED_FEATURES_BITS << 8 | VERSION_NUM); }
  ...
  [[nodiscard]] bool is_legacy_mode() const { return (_controller_bram[ADDR_CTL_REG] & (1 << CTL_REG_LEGACY_MODE_BIT)) != 0; }

  [[nodiscard]] bool is_force_fan() const { return (_controller_bram[ADDR_CTL_REG] & (1 << CTL_REG_FORCE_FAN_BIT)) != 0; }

  [[nodiscard]] bool is_stm_mode() const { return (_controller_bram[ADDR_CTL_REG] & (1 << CTL_REG_OP_MODE_BIT)) != 0; }
  [[nodiscard]] bool is_stm_gain_mode() const { return (_controller_bram[ADDR_CTL_REG] & (1 << CTL_REG_STM_GAIN_MODE_BIT)) != 0; }

  [[nodiscard]] uint16_t silencer_cycle() const { return _controller_bram[ADDR_SILENT_CYCLE]; }

  [[nodiscard]] uint16_t silencer_step() const { return _controller_bram[ADDR_SILENT_STEP]; }

  [[nodiscard]] std::array<uint16_t, NUM_TRANS_IN_UNIT> cycles() const {
    std::array<uint16_t, NUM_TRANS_IN_UNIT> cycles{};
    for (size_t i = 0; i < NUM_TRANS_IN_UNIT; i++) cycles[i] = _controller_bram[ADDR_CYCLE_BASE + i];
    return cycles;
  }

  [[nodiscard]] std::array<uint16_t, NUM_TRANS_IN_UNIT> mod_delays() const {
    std::array<uint16_t, NUM_TRANS_IN_UNIT> delays{};
    for (size_t i = 0; i < NUM_TRANS_IN_UNIT; i++) delays[i] = _controller_bram[ADDR_MOD_DELAY_BASE + i];
    return delays;
  }
  ...
  [[nodiscard]] std::pair<std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>>, std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>>>
  drives() const {
    if (is_stm_mode()) {
      if (is_stm_gain_mode()) {
 ...
 private:
  [[nodiscard]] std::array<driver::Duty, NUM_TRANS_IN_UNIT> normal_duty() const {
    std::array<driver::Duty, NUM_TRANS_IN_UNIT> d{};
    for (size_t i = 0; i < NUM_TRANS_IN_UNIT; i++) d[i] = driver::Duty{_normal_op_bram[2 * i + 1]};
    return d;
  }

  [[nodiscard]] std::array<driver::Phase, NUM_TRANS_IN_UNIT> normal_phase() const {
    std::array<driver::Phase, NUM_TRANS_IN_UNIT> d{};
    for (size_t i = 0; i < NUM_TRANS_IN_UNIT; i++) d[i] = driver::Phase{_normal_op_bram[2 * i]};
    return d;
  }

  [[nodiscard]] std::array<driver::Duty, NUM_TRANS_IN_UNIT> legacy_duty() const {
    std::array<driver::Duty, NUM_TRANS_IN_UNIT> d{};
  ...
  [[nodiscard]] std::array<driver::Phase, NUM_TRANS_IN_UNIT> legacy_phase() const {
    std::array<driver::Phase, NUM_TRANS_IN_UNIT> d{};
    for (size_t i = 0; i < NUM_TRANS_IN_UNIT; i++) {
  ...
  [[nodiscard]] std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>> gain_stm_normal_duty() const {
    const auto cycle = stm_cycle();
    std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>> v;
    v.reserve(cycle);
    for (size_t i = 0; i < cycle; i++) {
      std::array<driver::Duty, NUM_TRANS_IN_UNIT> d{};
      for (size_t j = 0; j < NUM_TRANS_IN_UNIT; j++) d[j] = driver::Duty{_stm_op_bram[512 * i + 2 * j + 1]};
      v.emplace_back(d);
    }
    return v;
  }

  [[nodiscard]] std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>> gain_stm_normal_phase() const {
    const auto cycle = stm_cycle();
    std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>> v;
    v.reserve(cycle);
    for (size_t i = 0; i < cycle; i++) {
      std::array<driver::Phase, NUM_TRANS_IN_UNIT> d{};
      for (size_t j = 0; j < NUM_TRANS_IN_UNIT; j++) d[j] = driver::Phase{_stm_op_bram[512 * i + 2 * j]};
      v.emplace_back(d);
    }
    return v;
  }

  [[nodiscard]] std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>> gain_stm_legacy_duty() const {
    const auto cycle = stm_cycle();
    std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>> v;
    v.reserve(cycle);
    for (size_t i = 0; i < cycle; i++) {
  ...
  [[nodiscard]] std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>> gain_stm_legacy_phase() const {
    const auto cycle = stm_cycle();
    std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>> v;
    v.reserve(cycle);
    for (size_t i = 0; i < cycle; i++) {
  ...
  [[nodiscard]] std::vector<std::array<driver::Duty, NUM_TRANS_IN_UNIT>> point_stm_duty() const {
    const auto cycle = stm_cycle();
  ...
  [[nodiscard]] std::vector<std::array<driver::Phase, NUM_TRANS_IN_UNIT>> point_stm_phase() const {
    const auto cycle = stm_cycle();
    const auto ultrasound_cycles = cycles();
