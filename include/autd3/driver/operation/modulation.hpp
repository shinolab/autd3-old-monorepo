...
struct Modulation final : Operation {
  void init() override { sent = 0; }

  void pack(TxDatagram& tx) override {
    if (size > MOD_BUF_SIZE_MAX) throw std::runtime_error("Modulation buffer overflow");
    ...
    const auto mod_size = (std::min)(size - sent, max_size);
    const auto is_last_frame = sent + mod_size == size;
