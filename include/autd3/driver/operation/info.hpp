...
struct CPUVersionMajor final {
  static void pack(TxDatagram& tx) {
    ...
    tx.header().cpu_flag = static_cast<CPUControlFlags>(MSG_RD_CPU_VERSION_MAJOR);  // For backward compatibility before 1.9
    tx.num_bodies = 0;
  }
};
...
struct FPGAVersionMajor final {
  static void pack(TxDatagram& tx) {
    ...
    tx.header().cpu_flag = static_cast<CPUControlFlags>(MSG_RD_FPGA_VERSION_MAJOR);  // For backward compatibility before 1.9
    tx.num_bodies = 0;
  }
};
...
struct FPGAFunctions final {
  static void pack(TxDatagram& tx) {
    ...
    tx.header().cpu_flag = static_cast<CPUControlFlags>(MSG_RD_FPGA_FUNCTION);  // For backward compatibility before 1.9
    tx.num_bodies = 0;
  }
};
}  // namespace autd3::driver
