 ...
 public:
  ...
  bool modulation(const uint8_t msg_id, const std::vector<uint8_t>& mod_data, size_t& sent, const uint32_t freq_div, TxDatagram& tx) const override {
    ...
    if (is_first_frame) {
      if (freq_div < v2_4::MOD_SAMPLING_FREQ_DIV_MIN) {
        spdlog::error("Modulation frequency division is oud of range. Minimum is {}, but you use {}.", v2_4::MOD_SAMPLING_FREQ_DIV_MIN, freq_div);
        return false;
      }
  ...
  bool config_silencer(const uint8_t msg_id, const uint16_t cycle, const uint16_t step, TxDatagram& tx) const override {
    if (cycle < v2_4::SILENCER_CYCLE_MIN) {
      spdlog::error("Silencer cycle is oud of range. Minimum is {}, but you use {}.", v2_4::SILENCER_CYCLE_MIN, cycle);
      return false;
    }
  ...
  bool point_stm_body(const std::vector<std::vector<STMFocus>>& points, size_t& sent, const size_t total_size, const uint32_t freq_div,
    ...
    if (sent == 0) {
      if (freq_div < v2_4::POINT_STM_SAMPLING_FREQ_DIV_MIN) {
        spdlog::error("STM frequency division is oud of range. Minimum is {}, but you use {}.", v2_4::POINT_STM_SAMPLING_FREQ_DIV_MIN, freq_div);
        return false;
      }
  ...
  bool gain_stm_legacy_body(const std::vector<std::vector<driver::Drive>>& drives, size_t& sent, const uint32_t freq_div, const GainSTMMode mode,
    ...
    if (sent == 0) {
      if (freq_div < v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN) {
        spdlog::error("STM frequency division is oud of range. Minimum is {}, but you use {}.", v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN,
                      freq_div);
        return false;
      }
...
#pragma warning(pop)
    ...
    if (sent == 0) {
      if (freq_div < v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN) {
        spdlog::error("STM frequency division is oud of range. Minimum is {}, but you use {}.", v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN, freq_div);
        return false;
      }
      tx.header().cpu_flag.set(CPUControlFlags::STM_BEGIN);
      for (size_t i = 0; i < tx.size(); i++) {
