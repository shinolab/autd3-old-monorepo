...
class DriverV2_4 final : public Driver {
 public:
  ...
  void modulation(const uint8_t msg_id, const std::vector<uint8_t>& mod_data, size_t& sent, const uint32_t freq_div, TxDatagram& tx) const override {
    if (mod_data.size() > v2_4::MOD_BUF_SIZE_MAX) throw std::runtime_error("Modulation buffer overflow");
    ...
    if (is_first_frame) {
      if (freq_div < v2_4::MOD_SAMPLING_FREQ_DIV_MIN)
        throw std::runtime_error("Modulation frequency division is oud of range. Minimum is " + std::to_string(v2_4::MOD_SAMPLING_FREQ_DIV_MIN) +
  ...
  void config_silencer(const uint8_t msg_id, const uint16_t cycle, const uint16_t step, TxDatagram& tx) const override {
    if (cycle < v2_4::SILENCER_CYCLE_MIN)
      throw std::runtime_error("Silencer cycle is oud of range. Minimum is " + std::to_string(v2_4::SILENCER_CYCLE_MIN) + ", but you use " +
                               std::to_string(cycle));
  ...
  void point_stm_body(const std::vector<std::vector<STMFocus>>& points, size_t& sent, const size_t total_size, const uint32_t freq_div,
    ...
    if (total_size > v2_4::POINT_STM_BUF_SIZE_MAX) throw std::runtime_error("PointSTM out of buffer");

    if (points.empty() || points[0].empty()) return;

    if (sent == 0) {
      if (freq_div < v2_4::POINT_STM_SAMPLING_FREQ_DIV_MIN)
        throw std::runtime_error("STM frequency division is oud of range. Minimum is " + std::to_string(v2_4::POINT_STM_SAMPLING_FREQ_DIV_MIN) +
                                 ", but you use " + std::to_string(freq_div));
  ...
  void gain_stm_legacy_body(const std::vector<std::vector<driver::Drive>>& drives, size_t& sent, const uint32_t freq_div, const GainSTMMode mode,
    ...
    if (drives.size() > v2_4::GAIN_STM_LEGACY_BUF_SIZE_MAX) throw std::runtime_error("GainSTM out of buffer");

    bool is_last_frame = false;
    if (sent == 0) {
      if (freq_div < v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN)
        throw std::runtime_error("STM frequency division is oud of range. Minimum is " + std::to_string(v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN) +
                                 ", but you use " + std::to_string(freq_div));
  ...
  void gain_stm_normal_phase(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
    ...
    if (drives.size() > v2_4::GAIN_STM_BUF_SIZE_MAX) throw std::runtime_error("GainSTM out of buffer");
...
#pragma warning(pop)
    ...
    if (sent == 0) {
      if (freq_div < v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN)
        throw std::runtime_error("STM frequency division is oud of range. Minimum is " + std::to_string(v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN) +
                                 ", but you use " + std::to_string(freq_div));
      tx.header().cpu_flag.set(CPUControlFlags::STM_BEGIN);
      for (size_t i = 0; i < tx.size(); i++) {
        tx.bodies()[i].gain_stm_head().set_freq_div(freq_div);
