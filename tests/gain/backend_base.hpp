...
#define AUTD3_BACKEND_TEST(TestName, BackendType)                                                   \
  ...
  TEST(TestName, set) {                                                                             \
    ...
    VectorXc a(m);                                                                                  \
                                                                                                    \
    backend->set(m / 2, complex(10.0, 5.0), a);                                                     \
    backend->to_host(a);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, dot) {                                                                             \
    ...
    for (Eigen::Index i = 0; i < n; i++) expected += std::conj(a(i)) * b(i);                        \
                                                                                                    \
    ASSERT_NEAR_COMPLEX(backend->dot(a, b), expected, 1e-6);                                        \
  }                                                                                                 \
                                                                                                    \
  TEST(TestName, dot_real) {                                                                        \
    ...
    for (Eigen::Index i = 0; i < n; i++) expected += a(i) * b(i);                                   \
                                                                                                    \
    ASSERT_NEAR(backend->dot(a, b), expected, 1e-6);                                                \
  }                                                                                                 \
                                                                                                    \
  TEST(TestName, add_vector) {                                                                      \
    ...
    backend->add(std::complex(2.0, 0.0), aa, b);                                                    \
    backend->to_host(b);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, mul_matrix) {                                                                      \
    ...
    backend->mul(TRANSPOSE::NO_TRANS, TRANSPOSE::NO_TRANS, ONE, a, b, ZERO, c);                     \
    backend->to_host(c);                                                                            \
                                                                                                    \
    ...
    backend->mul(TRANSPOSE::CONJ_TRANS, TRANSPOSE::TRANS, 2.0 * ONE, aa, bb, ONE, c);               \
    backend->to_host(c);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, mul_vec) {                                                                         \
    ...
    backend->mul(TRANSPOSE::NO_TRANS, ONE, a, b, ZERO, c);                                          \
    backend->to_host(c);                                                                            \
                                                                                                    \
    ...
    for (Eigen::Index i = 0; i < n; i++) ASSERT_NEAR_COMPLEX(c(i), expected(i), 1e-6);              \
                                                                                                    \
    MatrixXc aa = MatrixXc::Random(m, n);                                                           \
    backend->mul(TRANSPOSE::CONJ_TRANS, 3.0 * ONE, aa, b, ONE, c);                                  \
    backend->to_host(c);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, mul_matrix_real) {                                                                 \
    ...
    backend->mul(TRANSPOSE::NO_TRANS, TRANSPOSE::NO_TRANS, 1.0, a, b, 0.0, c);                      \
    backend->to_host(c);                                                                            \
                                                                                                    \
    ...
    backend->mul(TRANSPOSE::TRANS, TRANSPOSE::TRANS, 2.0, aa, bb, 1.0, c);                          \
    backend->to_host(c);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, mul_vec_real) {                                                                    \
    ...
    backend->mul(TRANSPOSE::NO_TRANS, 1.0, a, b, 0.0, c);                                           \
    backend->to_host(c);                                                                            \
                                                                                                    \
    ...
    for (Eigen::Index i = 0; i < n; i++) ASSERT_NEAR(c(i), expected(i), 1e-6);                      \
                                                                                                    \
    MatrixXd aa = MatrixXd::Random(m, n);                                                           \
    backend->mul(TRANSPOSE::TRANS, 3.0, aa, b, 1.0, c);                                             \
    backend->to_host(c);                                                                            \
                                                                                                    \
  ...
  TEST(TestName, pseudo_inverse_svd) {                                                              \
    ...
    MatrixXc tmp = a;                                                                               \
    backend->pseudo_inverse_svd(tmp, 0.0, u, s, vt, buf, b);                                        \
                                                                                                    \
