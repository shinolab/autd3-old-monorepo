...
TEST(DriverV2_6Driver, operation_modulation_v2_6) {
  ...
  ASSERT_TRUE(driver.modulation(1, mod_data, sent, 580, tx));
  ...
  ASSERT_TRUE(driver.modulation(0xFF, mod_data, sent, 580, tx));
  ...
  ASSERT_TRUE(driver.modulation(0xF0, mod_data, sent, 580, tx));
...
TEST(DriverV2_6Driver, operation_config_silencer_v2_6) {
  ...
  ASSERT_TRUE(driver.config_silencer(1, 522, 4, tx));
  ASSERT_EQ(tx.header().msg_id, 1);
  ASSERT_FALSE(tx.header().cpu_flag.contains(CPUControlFlags::Mod));
  ASSERT_FALSE(tx.header().cpu_flag.contains(CPUControlFlags::ConfigSync));
