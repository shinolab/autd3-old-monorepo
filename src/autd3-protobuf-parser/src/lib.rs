...
use std::{error::Error, rc::Rc, sync::RwLock};
...
impl ToMessage for autd3::modulation::Static {
    type Message = DatagramHeader;

    #[allow(clippy::unnecessary_cast)]
    fn to_msg(&self) -> Self::Message {
        Self::Message {
            header: Some(datagram_header::Header::Modulation(Modulation {
...
impl ToMessage for autd3::gain::Focus {
    type Message = DatagramBody;

    #[allow(clippy::unnecessary_cast)]
    fn to_msg(&self) -> Self::Message {
        Self::Message {
            body: Some(datagram_body::Body::Gain(Gain {
...
impl ToMessage for autd3::gain::TransducerTest {
    ...
    fn to_msg(&self) -> Self::Message {
        ...
        Self::Message {
            body: Some(datagram_body::Body::Gain(Gain {
                gain: Some(gain::Gain::TransTest(TransducerTest { drives: drives })),
            })),
        }
    }
}
...
impl ToMessage for autd3_core::silencer_config::SilencerConfig {
    ...
    fn to_msg(&self) -> Self::Message {
        Self::Message {
            header: Some(datagram_header::Header::SilencerConfig(SilencerConfig {
                step: self.step() as _,
            })),
        }
    }
    ...
    fn from_msg(msg: &Grouped) -> Self {
        msg.groups.iter().fold(Self::new(), |acc, v| {
            if let Some(g) = &v.gain {
                match &g.gain {
                    Some(gain::Gain::Focus(g)) => acc.add::<autd3::gain::Focus>(
                        v.id as _,
                        autd3::gain::Focus::from_msg(g).into(),
                    ),
                    Some(gain::Gain::Bessel(g)) => acc.add::<autd3::gain::Bessel>(
                        v.id as _,
                        autd3::gain::Bessel::from_msg(g).into(),
                    ),
                    Some(gain::Gain::Null(g)) => acc
                        .add::<autd3::gain::Null>(v.id as _, autd3::gain::Null::from_msg(g).into()),
                    Some(gain::Gain::Plane(g)) => acc.add::<autd3::gain::Plane>(
                        v.id as _,
                        autd3::gain::Plane::from_msg(g).into(),
                    ),
                    Some(gain::Gain::TransTest(g)) => acc.add::<autd3::gain::TransducerTest>(
                        v.id as _,
                        autd3::gain::TransducerTest::from_msg(g).into(),
                    ),
                    ...
                    Some(gain::Gain::Gs(g)) => acc.add::<autd3_gain_holo::GS<_>>(
                        v.id as _,
                        autd3_gain_holo::GS::<autd3_gain_holo::NalgebraBackend>::from_msg(g).into(),
                    ),
                    ...
                    Some(gain::Gain::Lm(g)) => acc.add::<autd3_gain_holo::LM<_>>(
                        v.id as _,
                        autd3_gain_holo::LM::<autd3_gain_holo::NalgebraBackend>::from_msg(g).into(),
                    ),
                    Some(gain::Gain::Greedy(g)) => acc.add::<autd3_gain_holo::Greedy>(
                        v.id as _,
                        autd3_gain_holo::Greedy::from_msg(g).into(),
                    ),
                    _ => acc,
                }
            } else {
...
pub fn send_special<T: autd3_core::geometry::Transducer, L: autd3_core::link::Link<T>>(
    autd: &mut autd3::Controller<T, L>,
    msg: &SpecialData,
...
pub fn send_header<T: autd3_core::geometry::Transducer, L: autd3_core::link::Link<T>>(
    autd: &mut autd3::Controller<T, L>,
    header: &DatagramHeader,
) -> Result<bool, AUTDProtoBufError> {
    Ok(match &header.header {
        Some(datagram_header::Header::SilencerConfig(msg)) => {
            autd.send(autd3::prelude::SilencerConfig::from_msg(msg))
        }
        Some(datagram_header::Header::Modulation(modulation)) => match &modulation.modulation {
...
pub fn send_body<T: autd3_core::geometry::Transducer, L: autd3_core::link::Link<T>>(
    autd: &mut autd3::Controller<T, L>,
    body: &DatagramBody,
) -> Result<bool, AUTDProtoBufError> {
    Ok(match &body.body {
        Some(datagram_body::Body::Gain(gain)) => match &gain.gain {
    ...
    > ecat_light_server::EcatLight for LightweightServer<L, F>
{
    ...
    async fn send_header(
        &self,
        req: Request<DatagramHeader>,
    ) -> Result<Response<SendLightResponse>, Status> {
        if let Some(autd) = self.autd.write().unwrap().as_mut() {
            match send_header(autd, &req.into_inner()) {
                Ok(_) => Ok(Response::new(SendLightResponse {
                    success: true,
                    msg: String::new(),
                })),
    ...
    async fn send_body(
        &self,
        req: Request<DatagramBody>,
    ) -> Result<Response<SendLightResponse>, Status> {
        if let Some(autd) = self.autd.write().unwrap().as_mut() {
            match send_body(autd, &req.into_inner()) {
                Ok(_) => Ok(Response::new(SendLightResponse {
                    success: true,
                    msg: String::new(),
                })),
    ...
    async fn send_special(
        &self,
        req: Request<SpecialData>,
    ) -> Result<Response<SendLightResponse>, Status> {
        if let Some(autd) = self.autd.write().unwrap().as_mut() {
            match send_special(autd, &req.into_inner()) {
                Ok(_) => Ok(Response::new(SendLightResponse {
                    success: true,
                    msg: String::new(),
                })),
