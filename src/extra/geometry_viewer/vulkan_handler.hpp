...
class VulkanHandler {
 public:
  ...
  void create_texture_image(const uint8_t* image_buffer, const uint32_t image_len) {
    ...
    auto [texture_image, texture_image_memory] = create_image(
        static_cast<uint32_t>(tex_width), static_cast<uint32_t>(tex_height), _mip_levels, vk::SampleCountFlagBits::e1, vk::Format::eR8G8B8A8Srgb,
 ...
 private:
  ...
  static inline const std::vector device_extensions = {VK_KHR_SWAPCHAIN_EXTENSION_NAME, "VK_KHR_portability_subset"};
