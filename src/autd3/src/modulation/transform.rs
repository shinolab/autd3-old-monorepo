...
#[derive(Modulation)]
pub struct ModulationTransform<M: Modulation, F: Fn(usize, &float) -> float> {
    m: M,
    freq_div: u32,
    f: F,
}
...
pub trait Transform<M: Modulation> {
    fn with_transform<F: Fn(usize, &float) -> float>(self, f: F) -> ModulationTransform<M, F>;
}
...
impl<M: Modulation, F: Fn(usize, &float) -> float> Modulation for ModulationTransform<M, F> {
