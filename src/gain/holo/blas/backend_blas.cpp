...
void BLASBackend::pow(const VectorXd& src, const double p, VectorXd& dst) { dst = src.array().pow(p); }
...
double BLASBackend::max_element(const VectorXd& src) { return src.maxCoeff(); }

void BLASBackend::scale(const complex value, VectorXc& dst) { AUTD_ZSCAL(static_cast<int>(dst.size()), &value, dst.data(), 1); }
void BLASBackend::scale(const double value, VectorXd& dst) { AUTD_DSCAL(static_cast<int>(dst.size()), value, dst.data(), 1); }
...
double BLASBackend::dot(const VectorXd& a, const VectorXd& b) { return AUTD_DOT(static_cast<int>(a.size()), a.data(), 1, b.data(), 1); }
...
void BLASBackend::mul(const TRANSPOSE trans_a, const TRANSPOSE trans_b, const double alpha, const MatrixXd& a, const MatrixXd& b, const double beta,
                      MatrixXd& c) {
  const auto lda = static_cast<int>(a.rows());
  const auto ldb = static_cast<int>(b.rows());
  const auto ldc = trans_a == TRANSPOSE::NO_TRANS ? static_cast<int>(a.rows()) : static_cast<int>(a.cols());
...
void BLASBackend::mul(const TRANSPOSE trans_a, const double alpha, const MatrixXd& a, const VectorXd& b, const double beta, VectorXd& c) {
  const auto m = static_cast<int>(a.rows());
  const auto n = static_cast<int>(a.cols());
  const auto lda = m;
