...
uint8_t get_id() noexcept {
  static std::atomic id{autd3::driver::MSG_NORMAL_BEGINNING};
  if (uint8_t expected = autd3::driver::MSG_NORMAL_END; !id.compare_exchange_weak(expected, autd3::driver::MSG_NORMAL_BEGINNING)) id.fetch_add(0x01);
  return id.load();
}
}  // namespace
...
template <typename T>
core::Geometry<T>& Controller<T>::geometry() noexcept {
  return _geometry;
}

template <typename T>
bool Controller<T>::config_silencer(const SilencerConfig config) {
  _tx_buf.clear();
...
template <typename T>
std::vector<driver::FirmwareInfo> Controller<T>::firmware_infos() {
  const auto check_ack_ = check_ack;
  check_ack = true;
...
template <typename T>
bool Controller<T>::send(core::DatagramBody<T>&& body) {
  auto& b = body;
  return send(b);
}
...
template <typename T>
bool Controller<T>::send(core::DatagramHeader&& header, core::DatagramBody<T>&& body) {
  auto& h = header;
  auto& b = body;
  return send(h, b);
}
...
template <typename T>
bool Controller<T>::wait_msg_processed(const size_t max_trial) {
  if (!check_ack) {
    return true;
  }
  const auto msg_id = _tx_buf.header().msg_id;
  const auto wait = gsl::narrow_cast<uint64_t>(
      std::ceil(driver::EC_TRAFFIC_DELAY * 1000.0 / static_cast<double>(driver::EC_DEVICE_PER_FRAME) * static_cast<double>(_geometry.num_devices())));
  auto success = false;
...
template autd3::Controller<autd3::core::LegacyTransducer>;
template autd3::Controller<autd3::core::NormalTransducer>;
