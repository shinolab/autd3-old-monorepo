...
void DriverV2_2::point_stm_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
size_t DriverV2_2::point_stm_send_size(const size_t total_size, const size_t sent) const noexcept {
...
bool DriverV2_2::point_stm_body(const std::vector<std::vector<STMFocus>>& points, size_t& sent, const size_t total_size, const uint32_t freq_div,
                                const double sound_speed, TxDatagram& tx) const {
  if (total_size > v2_2::POINT_STM_BUF_SIZE_MAX) {
  ...
  if (sent == 0) {
    if (freq_div < v2_2::POINT_STM_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("STM frequency division is out of range. Minimum is {}, but you use {}.", v2_2::POINT_STM_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
...
bool DriverV2_2::gain_stm_legacy_body(const std::vector<std::vector<driver::Drive>>& drives, size_t& sent, const uint32_t freq_div,
...
bool DriverV2_2::gain_stm_normal_phase(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                       const GainSTMMode mode, TxDatagram& tx) const {
...
bool DriverV2_2::gain_stm_normal_duty(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                      const GainSTMMode mode, TxDatagram& tx) const {
  if (drives.size() > v2_2::GAIN_STM_BUF_SIZE_MAX) {
    spdlog::error("GainSTM out of buffer");
    return false;
