...
void DriverV2_2::focus_stm_initialer(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
bool DriverV2_2::focus_stm_subsequent(const std::vector<std::vector<STMFocus>>& points, size_t& sent, const size_t total_size,
