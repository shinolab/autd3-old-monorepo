...
uint8_t autd3::driver::DriverV2_4::version_num() const noexcept { return 0x84; }
...
size_t DriverV2_4::point_stm_send_size(const size_t total_size, const size_t sent) const noexcept {
...
bool DriverV2_4::gain_stm_legacy_body(const std::vector<std::vector<driver::Drive>>& drives, size_t& sent, const uint32_t freq_div,
...
bool DriverV2_4::gain_stm_normal_phase(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                       const GainSTMMode mode, TxDatagram& tx) const {
...
bool DriverV2_4::gain_stm_normal_duty(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                      const GainSTMMode mode, TxDatagram& tx) const {
  if (drives.size() > v2_4::GAIN_STM_BUF_SIZE_MAX) {
    spdlog::error("GainSTM out of buffer");
    return false;
