...
uint8_t DriverV2_4::version_num() const noexcept { return 0x84; }

void DriverV2_4::clear(TxDatagram& tx) const noexcept {
  tx.header().msg_id = MSG_CLEAR;
  tx.num_bodies = 0;
}

void DriverV2_4::null_header(const uint8_t msg_id, TxDatagram& tx) const noexcept {
...
void DriverV2_4::null_body(TxDatagram& tx) const noexcept {
...
bool DriverV2_4::modulation(const uint8_t msg_id, const std::vector<uint8_t>& mod_data, size_t& sent, const uint32_t freq_div, TxDatagram& tx) const {
  if (mod_data.size() > v2_4::MOD_BUF_SIZE_MAX) {
  ...
  if (is_first_frame) {
    if (freq_div < v2_4::MOD_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("Modulation frequency division is out of range. Minimum is {}, but you use {}.", v2_4::MOD_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
...
bool DriverV2_4::config_silencer(const uint8_t msg_id, const uint16_t cycle, const uint16_t step, TxDatagram& tx) const {
  if (cycle < v2_4::SILENCER_CYCLE_MIN) {
    spdlog::error("Silencer cycle is out of range. Minimum is {}, but you use {}.", v2_4::SILENCER_CYCLE_MIN, cycle);
    return false;
  }
...
void DriverV2_4::normal_legacy_header(TxDatagram& tx) noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WriteBody);
  tx.header().cpu_flag.remove(CPUControlFlags::ModDelay);
...
void DriverV2_4::normal_legacy_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WriteBody);
  tx.header().cpu_flag.remove(CPUControlFlags::ModDelay);
...
void DriverV2_4::normal_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WriteBody);
  tx.header().cpu_flag.remove(CPUControlFlags::ModDelay);
...
void DriverV2_4::normal_duty_body(const std::vector<Drive>& drives, const std::vector<uint16_t>& cycles, TxDatagram& tx) const noexcept {
...
void DriverV2_4::normal_phase_body(const std::vector<Drive>& drives, const std::vector<uint16_t>& cycles, TxDatagram& tx) const noexcept {
...
void DriverV2_4::gain_stm_legacy_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WriteBody);
  tx.header().cpu_flag.remove(CPUControlFlags::ModDelay);
  tx.header().cpu_flag.remove(CPUControlFlags::STMBegin);
  tx.header().cpu_flag.remove(CPUControlFlags::STMEnd);
...
bool DriverV2_4::gain_stm_legacy_body(const std::vector<std::vector<Drive>>& drives, size_t& sent, const uint32_t freq_div, const GainSTMMode mode,
                                      std::optional<uint16_t>, std::optional<uint16_t>, TxDatagram& tx) const {
  if (drives.size() > v2_4::GAIN_STM_LEGACY_BUF_SIZE_MAX) {
  ...
  if (sent == 0) {
    if (freq_div < v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("STM frequency division is out of range. Minimum is {}, but you use {}.", v2_4::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
...
void DriverV2_4::gain_stm_normal_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WriteBody);
  tx.header().cpu_flag.remove(CPUControlFlags::ModDelay);
  tx.header().cpu_flag.remove(CPUControlFlags::STMBegin);
  tx.header().cpu_flag.remove(CPUControlFlags::STMEnd);
...
bool DriverV2_4::gain_stm_normal_phase(const std::vector<std::vector<Drive>>& drives, const std::vector<uint16_t>& cycles, const size_t sent,
                                       const uint32_t freq_div, const GainSTMMode mode, std::optional<uint16_t>, std::optional<uint16_t>,
                                       TxDatagram& tx) const {
  if (drives.size() > v2_4::GAIN_STM_BUF_SIZE_MAX) {
    spdlog::error("GainSTM out of buffer");
    return false;
  }
...
#pragma warning(pop)
  ...
  if (sent == 0) {
    if (freq_div < v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("STM frequency division is out of range. Minimum is {}, but you use {}.", v2_4::GAIN_STM_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
    tx.header().cpu_flag.set(CPUControlFlags::STMBegin);
    for (size_t i = 0; i < tx.num_devices(); i++) {
...
bool DriverV2_4::gain_stm_normal_duty(const std::vector<std::vector<Drive>>& drives, const std::vector<uint16_t>& cycles, const size_t sent,
                                      const uint32_t freq_div, const GainSTMMode mode, std::optional<uint16_t>, std::optional<uint16_t>,
                                      TxDatagram& tx) const {
  if (drives.size() > v2_4::GAIN_STM_BUF_SIZE_MAX) {
    spdlog::error("GainSTM out of buffer");
...
void DriverV2_4::force_fan(TxDatagram& tx, const bool value) const noexcept {
  if (value)
...
void DriverV2_4::reads_fpga_info(TxDatagram& tx, const bool value) const noexcept {
  if (value)
...
void DriverV2_4::cpu_version(TxDatagram& tx) const noexcept {
...
void DriverV2_4::fpga_version(TxDatagram& tx) const noexcept {
...
void DriverV2_4::fpga_functions(TxDatagram& tx) const noexcept {
