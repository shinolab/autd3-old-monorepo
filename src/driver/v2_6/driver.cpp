...
uint8_t autd3::driver::DriverV2_6::version_num() const noexcept { return 0x86; }

void DriverV2_6::clear(TxDatagram& tx) const noexcept {
  tx.header().msg_id = MSG_CLEAR;
  tx.num_bodies = 0;
}
void DriverV2_6::null_header(const uint8_t msg_id, TxDatagram& tx) const noexcept {
...
void DriverV2_6::null_body(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
  tx.num_bodies = 0;
}
void DriverV2_6::sync(const uint16_t* const cycles, TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::MOD);
  tx.header().cpu_flag.remove(CPUControlFlags::CONFIG_SILENCER);
...
void DriverV2_6::mod_delay(const uint16_t* const delays, TxDatagram& tx) const noexcept {
...
bool DriverV2_6::modulation(const uint8_t msg_id, const std::vector<uint8_t>& mod_data, size_t& sent, const uint32_t freq_div, TxDatagram& tx) const {
  ...
  if (is_first_frame) {
    if (freq_div < v2_6::MOD_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("Modulation frequency division is out of range. Minimum is {}, but you use {}.", v2_6::MOD_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
...
bool DriverV2_6::config_silencer(const uint8_t msg_id, const uint16_t cycle, const uint16_t step, TxDatagram& tx) const {
  if (cycle < v2_6::SILENCER_CYCLE_MIN) {
    spdlog::error("Silencer cycle is out of range. Minimum is {}, but you use {}.", v2_6::SILENCER_CYCLE_MIN, cycle);
    return false;
  }
...
void DriverV2_6::normal_legacy_header(TxDatagram& tx) noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
void DriverV2_6::normal_legacy_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
void DriverV2_6::normal_legacy_body(const std::vector<Drive>& drives, TxDatagram& tx) const noexcept {
...
void DriverV2_6::normal_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
void DriverV2_6::normal_duty_body(const std::vector<Drive>& drives, TxDatagram& tx) const noexcept {
...
void DriverV2_6::normal_phase_body(const std::vector<Drive>& drives, TxDatagram& tx) const noexcept {
...
void DriverV2_6::point_stm_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
...
size_t DriverV2_6::point_stm_send_size(const size_t total_size, const size_t sent) const noexcept {
...
bool DriverV2_6::point_stm_body(const std::vector<std::vector<STMFocus>>& points, size_t& sent, const size_t total_size, const uint32_t freq_div,
...
void DriverV2_6::gain_stm_legacy_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
  tx.header().cpu_flag.remove(CPUControlFlags::STM_BEGIN);
  tx.header().cpu_flag.remove(CPUControlFlags::STM_END);
...
bool DriverV2_6::gain_stm_legacy_body(const std::vector<std::vector<driver::Drive>>& drives, size_t& sent, const uint32_t freq_div,
  ...
  if (sent == 0) {
    if (freq_div < v2_6::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("STM frequency division is out of range. Minimum is {}, but you use {}.", v2_6::GAIN_STM_LEGACY_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
...
void DriverV2_6::gain_stm_normal_header(TxDatagram& tx) const noexcept {
  tx.header().cpu_flag.remove(CPUControlFlags::WRITE_BODY);
  tx.header().cpu_flag.remove(CPUControlFlags::MOD_DELAY);
  tx.header().cpu_flag.remove(CPUControlFlags::STM_BEGIN);
  tx.header().cpu_flag.remove(CPUControlFlags::STM_END);
...
bool DriverV2_6::gain_stm_normal_phase(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                       const GainSTMMode mode, TxDatagram& tx) const {
...
#pragma warning(pop)
  ...
  if (sent == 0) {
    if (freq_div < v2_6::GAIN_STM_SAMPLING_FREQ_DIV_MIN) {
      spdlog::error("STM frequency division is out of range. Minimum is {}, but you use {}.", v2_6::GAIN_STM_SAMPLING_FREQ_DIV_MIN, freq_div);
      return false;
    }
    tx.header().cpu_flag.set(CPUControlFlags::STM_BEGIN);
    for (size_t i = 0; i < tx.size(); i++) {
...
bool DriverV2_6::gain_stm_normal_duty(const std::vector<std::vector<driver::Drive>>& drives, const size_t sent, const uint32_t freq_div,
                                      const GainSTMMode mode, TxDatagram& tx) const {
  if (drives.size() > v2_6::GAIN_STM_BUF_SIZE_MAX) {
    spdlog::error("GainSTM out of buffer");
    return false;
...
void DriverV2_6::fpga_version(TxDatagram& tx) const noexcept {
...
void DriverV2_6::fpga_functions(TxDatagram& tx) const noexcept {
